-- [[ Linux Kernel Development Support ]]
    ...
    if M.is_kernel_tree() then
      if vim.fn.filereadable('compile_commands.json') == 0 then
        if not vim.g.kernel_hint_shown then
          ...
          vim.notify('Linux kernel detected! Run :KernelGenCompileCommands for LSP support', vim.log.levels.INFO)
        end
      end
    end
  end,
...
-- Return empty table (no plugins to install)
return {}
